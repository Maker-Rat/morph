...
import numpy as np
import math
from csmt.models.multi_attention_forward import multi_head_attention_forward
# get body parts in Lafan1 and dog sets
from csmt.utils.utils import get_part_matrix, get_transformer_matrix, get_offset_part_matrix
# get body parts in Mixamo set
from csmt.utils.utils import getbodyparts, calselfmask
...
class MotionAE(nn.Module):
    def __init__(self, args, correspondence, njoints):
        ...
        # VAE heads — only active when use_vae=True
        self.use_vae = getattr(args, 'use_vae', False)
        if self.use_vae:
            latent_channels = args.transformer_latents * (len(correspondence) + 1)
            self.mu_head      = nn.Conv1d(latent_channels, latent_channels, kernel_size=1)
            self.log_var_head = nn.Conv1d(latent_channels, latent_channels, kernel_size=1)
    ...
    def forward(self, input, offset=None):
        z_det = self.enc(input)
        if self.use_vae:
            mu      = self.mu_head(z_det)
            log_var = self.log_var_head(z_det)
            z       = self.reparameterize(mu, log_var)
            result  = self.dec(z, offset)
            return z, mu, log_var, result
        else:
            result = self.dec(z_det, offset)
            return z_det, result

    def encode(self, input):
        """Encode without decoding. Returns (z, mu, log_var) if VAE else (z, None, None)."""
        z_det = self.enc(input)
        if self.use_vae:
            mu      = self.mu_head(z_det)
            log_var = self.log_var_head(z_det)
            z       = self.reparameterize(mu, log_var)
            return z, mu, log_var
        return z_det, None, None

    def outformat2input(self, output):
        # output shape : B T C  →  input shape B C T
...
class MotionEncoder(nn.Module):
    ...
    def __init__(self, args, correspondence, njoints, activation="gelu"):
        ...
        self.mask = get_part_matrix(correspondence, self.njoints_only + self.nroot_features)
        # Manually set all parts to also see the last 2 columns (linvel, quat scalars)
        self.mask[:, -self.nroot_features:] = 1

        self.dec_mask = get_part_matrix(correspondence, self.njoints_only) 

        # Joint angle projection: scalar → latent_dim  (2-layer, mirrors PositionalEncoding)
        hid_dim = 512
        self.joint_pos_encoder = PositionalEncoding(self.src_dim, self.latent_dim, self.dropout)

        # Root feature projections: each projects to latent_dim via 2-layer MLP
        self.linvel_proj = nn.Sequential(
            nn.Linear(3, hid_dim),
            nn.ReLU(),
        ...
        self.total_latent_dim = args.transformer_latents * (self.nparts + 1)  # nparts + 1 merged root stream

        self.joint_raw_dim = int(self.njoints_only * self.src_dim)
        self.raw_input_dim = int((self.njoints_only + self.nroot_features) * self.src_dim)  # +2 for root features
        ...
        # Residual conv: joint scalars only → joint_latent_dim channels only
        # (root token channels are zero in the residual, not corrupted by joint features)
        self.conv_residual = BodyPartConv(self.nparts, self.mask, self.raw_input_dim,
                                                   self.conv_input_dim, args.kernel_size, self.njoints_only + self.nroot_features,
                                                   stride=2, padding=padding, bias=True,
                                                   padding_mode=padding_mode, first_layer=True)

        # Temporal layers operate on full conv_input_dim (joints + linvel + quat channels)
        self.conv1 = BodyPartConv(self.nparts, self.mask, self.conv_input_dim,
                                                   self.conv_input_dim, args.kernel_size, self.njoints_only,
                                                   stride=2, padding=padding, bias=True,
    ...
    def forward(self, x):
        ...
        b_size, _, t_size = joint_x.shape
        BT = b_size * t_size

        # --- Joint tokens: project each scalar to latent_dim via PositionalEncoding ---
        j_num = self.njoints_only  # src_dim=1 so c_size == njoints_only
        joint_in = joint_x.reshape(b_size, j_num, self.src_dim, t_size) \
                           .transpose(0, 1).transpose(2, 3) \
                           .reshape(j_num, BT, self.src_dim)        # [njoints, BT, 1]
        joint_tokens = self.joint_pos_encoder(joint_in)              # [njoints, BT, latent_dim]

        # --- lin_vel token: project [B, 3, T] → [1, BT, latent_dim] ---
        ...
        target_t = final_parts.shape[2]
        linvel_latent = F.adaptive_avg_pool1d(linvel_latent, target_t)
        yaw_latent    = F.adaptive_avg_pool1d(yaw_latent,    target_t)

        # Merge linvel+yaw into single root stream by averaging
        root_latent = (linvel_latent + yaw_latent) / 2.0
        return torch.cat([final_parts, root_latent], dim=1)   # [B, (nparts+1)*D, T/4]
...
class MotionDecoder(nn.Module):
    def __init__(self, args, enc: MotionEncoder):
        ...
        self.nparts = enc.nparts
        self.latent_dim = enc.latent_dim
        self.njoints_only = enc.njoints_only

        # Channels belonging to joint body-part tokens (nparts)
        # These feed through the deconv layers to reconstruct joint angles
        self.joint_latent_dim = enc.latent_dim * enc.nparts
        self.root_latent_dim = enc.latent_dim

        # Dedicated output heads for root features, applied to their latent channels
        hid_dim = 512
        self.linvel_head = nn.Sequential(
        ...
        self.layers = nn.ModuleList()
        in_channels = enc.conv_input_dim  # deconv only sees joint channels

        # Deconvolutional and Upsampling layers (joint stream only)
        for i in range(self.num_layers):
            if i == self.num_layers - 1:
                last_layer = True
                out_channels = enc.joint_raw_dim
                mask_to_use = enc.dec_mask   # njoints_only * src_dim
            else:
                last_layer = False
                out_channels = self.joint_latent_dim
                mask_to_use = enc.mask

            layer_components = []
            layer_components.append(nn.Upsample(scale_factor=2, mode=args.upsampling, align_corners=False))
            layer_components.append(BodyPartConv(enc.nparts, mask_to_use, in_channels,
                                         out_channels, kernel_size, enc.njoints_only, stride=1, padding=padding,
    ...
    def forward(self, x, offset=None):
        # x: [B, (nparts+1)*D, T/4]  — joint stream + single root stream
        joint_x = x[:, :self.joint_latent_dim, :]   # [B, nparts*D, T/4]
        root_x  = x[:, self.joint_latent_dim:, :]   # [B, D, T/4]

        if self.add_offset and offset is not None:
            joint_x = joint_x + offset / 100
        for layer in self.layers:
            joint_x = layer(joint_x)

        joint_out = joint_x.transpose(1, 2)   # [B, T, njoints_only]
        T = joint_out.shape[1]

        root_in = root_x.permute(0, 2, 1)     # [B, T/4, D]

        linvel_out = self.linvel_head(root_in)
        linvel_out = F.interpolate(linvel_out.permute(0, 2, 1),
                                   size=T, mode='linear',
                                   align_corners=False).permute(0, 2, 1)  # [B, T, 3]

        yaw_out = self.yaw_head(root_in)
        yaw_out = F.interpolate(yaw_out.permute(0, 2, 1),
                                size=T, mode='linear',
...
class SkeletonEncoder(nn.Module):
    def __init__(self, args, correspondence, nbodies):
        ...
        if self.is_lafan1:    # Skeleton Encoder for biped and quadruped retargeting
            self.correspondence = correspondence
            self.nparts = len(correspondence)
            num_offsets = nbodies # remove the root joint translation
            input_dim = num_offsets * 3  # offsets dimension
            # Output includes root as virtual body part
